...
public class Path {
    ...
    public static Path createFastestPathFromNodes(Graph graph, List<Node> nodes)
        ...
        if (nodes.size() == 0) {
            return new Path(graph); // Empty path
        }
        else if (nodes.size() == 1) {
            return new Path(graph, nodes.get(0)); // Single-node path
        }

        for (int i = 0; i < nodes.size() - 1; ++i) {
            Node current = nodes.get(i);
            Node next = nodes.get(i + 1);
            List<Arc> successors = current.getSuccessors();
            Arc bestArc = null;
            double minTime = Double.POSITIVE_INFINITY;

            for (Arc arc : successors) {
                if (arc.getDestination().equals(next)
                        && arc.getMinimumTravelTime() < minTime) {
            ...
            if (bestArc == null) {
                throw new IllegalArgumentException("No arc connects node "
                        + current.getId() + " to node " + next.getId());
            }
    ...
    public static Path createShortestPathFromNodes(Graph graph, List<Node> nodes)
        ...
        for (int i = 0; i < nodes.size() - 1; ++i) {
            List<Arc> n = nodes.get(i).getSuccessors();
            Arc retenu = null;
            Float taille_chemin = Float.POSITIVE_INFINITY;
            for (int p = 0; p < n.size(); ++p) {
                if (taille_chemin > n.get(p).getLength() && n.get(p).getDestination()
                        .getId() == nodes.get(i + 1).getId()) {
                    taille_chemin = n.get(p).getLength();
                    retenu = n.get(p);
                }
            }
            if (taille_chemin == Float.POSITIVE_INFINITY)
    ...
    public boolean isValid() {
        if (arcs.isEmpty())
            return true;
        if (arcs.isEmpty() && graph.getNodes().size() == 1)
            return true;
        if (arcs.get(0).getOrigin().getId() != origin.getId())
            return false;

        for (int i = 0; i < arcs.size() - 1; ++i) {

            if (arcs.get(i).getDestination().getId() != arcs.get(i + 1).getOrigin()
                    .getId())
                return false;
        }
        return true;
    ...
    public double getTravelTime(double speed) {
        ...
        for (Arc a : arcs) {
            time_travel += a.getTravelTime(speed);
        }
        return time_travel;
    }
    ...
    public double getMinimumTravelTime() {
        float time_travel = 0;
        for (Arc a : arcs) {
            time_travel += a.getMinimumTravelTime();
        }
        return time_travel;
    }
}
